...
interface ReassignContactsDialogProps {
  isOpen: boolean
  onClose: () => void
  onReassignContacts: (agentId: string, contactIds: string[]) => void
...
export function ReassignContactsDialog({
  isOpen,
  onClose,
  onReassignContacts,
  selectedContactIds,
}: ReassignContactsDialogProps) {
  const [selectedAgent, setSelectedAgent] = useState("")

  const handleConfirm = () => {
    if (selectedAgent && selectedContactIds.length > 0) {
      onReassignContacts(selectedAgent, selectedContactIds)
      setSelectedAgent("")
    }
  }
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          You are Re-Assigning {selectedContactIds.length} Contact{selectedContactIds.length !== 1 ? "s" : ""}.
        </p>
